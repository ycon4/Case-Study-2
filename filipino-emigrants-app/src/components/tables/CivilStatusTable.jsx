...
function CivilStatusTable() {
  ...
  const initialFormState = { year: "", single: "", married: "", widower: "", separated: "", divorced: "", notReported: "" };
  const [form, setForm] = useState(initialFormState);
  const collectionRef = collections.civilStatus;
  ...
  const handleImport = async (event) => { const file = event.target.files[0]; if (!file) return; setIsUploading(true); try { const csvData = await importCSV(file); const batch = db.batch(); csvData.forEach(row => { const docRef = collectionRef.doc(); batch.set(docRef, { year: Number(row.YEAR) || 0, single: Number(row.Single) || 0, married: Number(row.Married) || 0, widower: Number(row.Widower) || 0, separated: Number(row.Separated) || 0, divorced: Number(row.Divorced) || 0, notReported: Number(row['Not Reported']) || 0 }); }); await batch.commit(); showNotification(`Successfully imported ${csvData.length} records!`, 'success'); fetchData(); } catch (error) { showNotification('Error importing CSV. Please check format.', 'error'); } finally { setIsUploading(false); event.target.value = ''; } };
  const handleExport = () => { if (filteredData.length === 0) { showNotification('No data to export', 'error'); return; } exportToCSV(filteredData, `civil-status-data-${new Date().toISOString().split('T')[0]}`); showNotification('Data exported successfully!', 'success'); };
  ...
  const statusFields = [ { label: 'Year', key: 'year' }, { label: 'Single', key: 'single' }, { label: 'Married', key: 'married' }, { label: 'Widower', key: 'widower' }, { label: 'Separated', key: 'separated' }, { label: 'Divorced', key: 'divorced' }, { label: 'Not Reported', key: 'notReported' }, ];
  ...
  return (
    <div>
      ...
      <h2 className="content-title">Civil Status - Data</h2>
      ...
      <div className="add-form">
        ...
        <div className="form-inputs">
          {statusFields.map(field => ( <input key={field.key} name={field.key} type="number" placeholder={field.label} value={form[field.key]} onChange={handleChange} className="form-input" /> ))}
        ...
        <table className="data-table">
          <thead>
            <tr>
              ...
              <th>Single</th><th>Married</th><th>Widower</th><th>Separated</th><th>Divorced</th><th>Not Reported</th>
            </tr>
          </thead>
          <tbody>
            {filteredData.map(item => (
              ...
              >
                ...
                <td>{item.single || 0}</td><td>{item.married || 0}</td><td>{item.widower || 0}</td><td>{item.separated || 0}</td><td>{item.divorced || 0}</td><td>{item.notReported || 0}</td>
              </tr>
            ))}
          </tbody>
        </table>
