...
function AgeTable() {
  ...
  const initialFormState = { year: "", age14Below: "", age15_19: "", age20_24: "", age25_29: "", age30_34: "", age35_39: "", age40_44: "", age45_49: "", age50_54: "", age55_59: "", age60_64: "", age65_69: "", age70Above: "", notReported: "" };
  const [form, setForm] = useState(initialFormState);
  const collectionRef = collections.age;
  ...
  const handleImport = async (event) => { const file = event.target.files[0]; if (!file) return; setIsUploading(true); try { const csvData = await importCSV(file); const batch = db.batch(); csvData.forEach(row => { const docRef = collectionRef.doc(); batch.set(docRef, { year: Number(row.Year) || 0, age14Below: Number(row['14 - Below']) || 0, age15_19: Number(row['15 - 19']) || 0, age20_24: Number(row['20 - 24']) || 0, age25_29: Number(row['25 - 29']) || 0, age30_34: Number(row['30 - 34']) || 0, age35_39: Number(row['35 - 39']) || 0, age40_44: Number(row['40 - 44']) || 0, age45_49: Number(row['45 - 49']) || 0, age50_54: Number(row['50 - 54']) || 0, age55_59: Number(row['55 - 59']) || 0, age60_64: Number(row['60 - 64']) || 0, age65_69: Number(row['65 - 69']) || 0, age70Above: Number(row['70 - Above']) || 0, notReported: Number(row['Not Reported / No Response']) || 0 }); }); await batch.commit(); showNotification(`Successfully imported ${csvData.length} records!`, 'success'); fetchData(); } catch (error) { showNotification('Error importing CSV. Please check format.', 'error'); } finally { setIsUploading(false); event.target.value = ''; } };
  const handleExport = () => { if (filteredData.length === 0) { showNotification('No data to export', 'error'); return; } exportToCSV(filteredData, `age-data-${new Date().toISOString().split('T')[0]}`); showNotification('Data exported successfully!', 'success'); };
  ...
  const ageFields = [ { label: 'Year', key: 'year' }, { label: '14 - Below', key: 'age14Below' }, { label: '15 - 19', key: 'age15_19' }, { label: '20 - 24', key: 'age20_24' }, { label: '25 - 29', key: 'age25_29' }, { label: '30 - 34', key: 'age30_34' }, { label: '35 - 39', key: 'age35_39' }, { label: '40 - 44', key: 'age40_44' }, { label: '45 - 49', key: 'age45_49' }, { label: '50 - 54', key: 'age50_54' }, { label: '55 - 59', key: 'age55_59' }, { label: '60 - 64', key: 'age60_64' }, { label: '65 - 69', key: 'age65_69' }, { label: '70 - Above', key: 'age70Above' }, { label: 'Not Reported', key: 'notReported' }, ];
  ...
  return (
    <div>
      ...
      <h2 className="content-title">Age - Data</h2>
      ...
      <div style={{ marginBottom: '20px', display: 'flex', gap: '10px', flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        {/* <-- REMOVED: Redundant external action buttons were here --> */}
      </div>
      ...
      <div className="add-form">
        ...
        <div className="form-inputs">
          {ageFields.map(field => ( <input key={field.key} name={field.key} type="number" placeholder={field.label} value={form[field.key]} onChange={handleChange} className="form-input" /> ))}
        ...
        <table className="data-table">
          <thead>
            <tr>
              ...
              <th>14-Below</th><th>15-19</th><th>20-24</th><th>25-29</th><th>30-34</th><th>35-39</th><th>40-44</th><th>45-49</th><th>50-54</th><th>55-59</th><th>60-64</th><th>65-69</th><th>70+</th><th>Not Rep.</th>
            </tr>
          </thead>
          <tbody>
            {filteredData.map(item => (
              ...
              >
                ...
                <td>{item.age14Below || 0}</td><td>{item.age15_19 || 0}</td><td>{item.age20_24 || 0}</td><td>{item.age25_29 || 0}</td><td>{item.age30_34 || 0}</td><td>{item.age35_39 || 0}</td><td>{item.age40_44 || 0}</td><td>{item.age45_49 || 0}</td><td>{item.age50_54 || 0}</td><td>{item.age55_59 || 0}</td><td>{item.age60_64 || 0}</td><td>{item.age65_69 || 0}</td><td>{item.age70Above || 0}</td><td>{item.notReported || 0}</td>
              </tr>
            ))}
          </tbody>
        </table>
