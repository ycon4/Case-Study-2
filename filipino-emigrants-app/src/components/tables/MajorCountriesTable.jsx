...
function MajorCountriesTable() {
  ...
  const initialFormState = { year: "", usa: "", canada: "", japan: "", australia: "", italy: "", newZealand: "", unitedKingdom: "", germany: "", southKorea: "", spain: "", others: "" };
  const [form, setForm] = useState(initialFormState);
  const collectionRef = collections.majorCountries;
  ...
  const handleImport = async (event) => { const file = event.target.files[0]; if (!file) return; setIsUploading(true); try { const csvData = await importCSV(file); const batch = db.batch(); csvData.forEach(row => { const docRef = collectionRef.doc(); batch.set(docRef, { year: Number(row.YEAR) || 0, usa: Number(row.USA) || 0, canada: Number(row.CANADA) || 0, japan: Number(row.JAPAN) || 0, australia: Number(row.AUSTRALIA) || 0, italy: Number(row.ITALY) || 0, newZealand: Number(row['NEW ZEALAND']) || 0, unitedKingdom: Number(row['UNITED KINGDOM']) || 0, germany: Number(row.GERMANY) || 0, southKorea: Number(row['SOUTH KOREA']) || 0, spain: Number(row.SPAIN) || 0, others: Number(row.OTHERS) || 0 }); }); await batch.commit(); showNotification(`Successfully imported ${csvData.length} records!`, 'success'); fetchData(); } catch (error) { showNotification('Error importing CSV. Please check format.', 'error'); } finally { setIsUploading(false); event.target.value = ''; } };
  const handleExport = () => { if (filteredData.length === 0) { showNotification('No data to export', 'error'); return; } exportToCSV(filteredData, `major-countries-data-${new Date().toISOString().split('T')[0]}`); showNotification('Data exported successfully!', 'success'); };
  ...
  const countryFields = [ { label: 'Year', key: 'year' }, { label: 'USA', key: 'usa' }, { label: 'Canada', key: 'canada' }, { label: 'Japan', key: 'japan' }, { label: 'Australia', key: 'australia' }, { label: 'Italy', key: 'italy' }, { label: 'New Zealand', key: 'newZealand' }, { label: 'United Kingdom', key: 'unitedKingdom' }, { label: 'Germany', key: 'germany' }, { label: 'South Korea', key: 'southKorea' }, { label: 'Spain', key: 'spain' }, { label: 'Others', key: 'others' }, ];
  ...
  return (
    <div>
      ...
      <h2 className="content-title">Major Countries - Data</h2>
      ...
      <div className="add-form">
        ...
        <div className="form-inputs">
          {countryFields.map(field => ( <input key={field.key} name={field.key} type="number" placeholder={field.label} value={form[field.key]} onChange={handleChange} className="form-input" /> ))}
        ...
        <table className="data-table">
          <thead>
            <tr>
              ...
              <th>USA</th><th>Canada</th><th>Japan</th><th>Australia</th><th>Italy</th><th>New Zealand</th><th>United Kingdom</th><th>Germany</th><th>South Korea</th><th>Spain</th><th>Others</th>
            </tr>
          </thead>
          <tbody>
            {filteredData.map(item => (
              ...
              >
                ...
                <td>{item.usa || 0}</td><td>{item.canada || 0}</td><td>{item.japan || 0}</td><td>{item.australia || 0}</td><td>{item.italy || 0}</td><td>{item.newZealand || 0}</td><td>{item.unitedKingdom || 0}</td><td>{item.germany || 0}</td><td>{item.southKorea || 0}</td><td>{item.spain || 0}</td><td>{item.others || 0}</td>
              </tr>
            ))}
          </tbody>
        </table>
